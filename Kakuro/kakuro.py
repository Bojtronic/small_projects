...
#Variable para saber si el juego esta iniciado

juego_iniciado=False

# Variable para almacenar el número seleccionado y el ultimo boton de numero presionado
...
datos_top={'Facil':[],'Medio':[],'Dificil':[],'Experto':[]}

texto_top = tk.Text(win, width=40, height=20,bg=gris_claro,fg='white')
...
def seleccionar_numero(numero):
    ...
    for i in botones_numeros:
        i.config(bg=gris_oscuro)
    for i in botones_numeros:
        if i['text']==str(numero):
            i.config(bg=gris_claro)
...
def revisar_partida():
    ...
    for fila in squares:
        row=[]
        for casilla in fila:
            if casilla.numero==None:
                row.append(0)
            else:
                row.append(casilla.numero)
        squares_formatted.append(row)
        
    for fila in partida:
        row=[]
        for casilla in fila:
            if casilla in [0,1000,2000,3000]:
                row.append(0)
            else:
                row.append(casilla)
        partida_formatted.append(row)
    if partida_formatted==squares_formatted:
        return True
    else:
        return False
...
def seleccionar_casilla(casilla):
    ...
    for i in botones_numeros:
        i.config(bg=gris_oscuro)
    if casilla.numero and numero_seleccionado:
        messagebox.showerror('Error','No puede seleccionar una casilla que ya contiene un valor')
    if numero_seleccionado==None and numero_seleccionado!=0:
        messagebox.showerror('Error','No puede seleccionar una casilla sin haber seleccionado un numero')
    ...
    if not casilla.numero and numero_seleccionado:
        ...
        jugadas_realizadas.append(casilla)
        jugadas_deshechas.clear()
    if not partida_llena():
        if revisar_partida():
            messagebox.showinfo('Ganador',' ¡EXCELENTE JUGADOR! TERMINÓ EL JUEGO CON ÉXITO')
            ...
            if configuracion['reloj']!=2:
                with open("kakuro2023top10.dat", mode="w") as file:
                    json.dump(datos_top, file)
            jugar()
        else:            
            messagebox.showinfo('Casi!','Buen intento, pero aun no esta del todo correcto')
...
def crear_cuadricula(nivel, numero_partida):
    global squares,partida, partida_actual
    ...
    for i in range(9):
        ...
        for j in range(9): 
            ...
            if partida[i][j] == 0:
                square.config(state='disabled')  # Deshabilita el boton
                square.configure(bg='#888888')
                texto_casilla = "0"

            
            elif partida[i][j] == 1000: # 1000 indica una pista de columna
                ...
                label = tk.Label(cuadricula, text=texto, font=('Arial', 10, 'bold'), fg='black', bg='#888888')
                label.place(x=label_x, y=label_y, anchor='center')
                                    
            elif partida[i][j] == 2000: # 2000 indica una pista de fila
                square.config(state='disabled')  
                square.configure(bg='#888888')
            ...
            elif partida[i][j] == 3000: # 3000 indica una pista de fila y de columna
                square.config(state='disabled')  
                square.configure(bg='#888888')
            ...
            row.append(square)
            row_partida_actual.append(texto_casilla)
        
        squares.append(row)
        partida_actual.append(row_partida_actual)
...
def eliminar_cuadricula():
    global squares
    label_cronometro.place_forget()
    # Eliminar etiquetas
    for row in squares:
        for square in row:
            square.destroy()  # Eliminar cada cuadro del canvas
...
def partida_llena():
    ...
    for r_i, row in enumerate(squares):
        for c_i,casilla in enumerate(row):
            if (casilla.numero==None or casilla.numero==0) and partida[r_i][c_i] not in [0,1000,2000,3000]:
                vacia=True
    return vacia
...
def iniciar_juego():
    ...
    if nombre.get()=='':
        ...
        segundos_entry.config(bg='#ffbba8')
        messagebox.showerror("Error", "Debe ingresar un tiempo, este debe ser unicamente en numeros enteros.")
    elif timer[0] != '' and int(timer[0])not in range(0,2):
        print('a')
        messagebox.showerror('Error','Debe ingresar un tiempo valido')
    elif timer[1] != '' and int(timer[1])not in range(0,60):
        print('b')
        messagebox.showerror('Error','Debe ingresar un tiempo valido')
    elif timer[2] != '' and int(timer[2])not in range(0,60):
        messagebox.showerror('Error','Debe ingresar un tiempo valido')
    else:
        ...
        try:
            ...
            messagebox.showerror('Error','Error, No hay partidas de este nivel')
            borrar_items()
            jugar()
...
def rehacer_jugada():
    if jugadas_deshechas:
        ...
        if ultima_jugada[1]not in [0,None]:
...
def borrar_juego():
    opcion = messagebox.askyesno("Opciones", "¿Desea borrar todas las casillas del juego?", 
...
def terminar_juego():
    if juego_iniciado:
        opcion = messagebox.askyesno("Opciones", "¿Desea terminar el juego?", 
                            detail="Selecciona una opción", 
                            icon="question")
        if opcion:
            iniciar_juego()
    else:
        messagebox.showerror('Error', 'Error: La partida aun no ha iniciado')
        print('Terminar Juego')

def top_10():
    pausar_reanudar()
        # Cargar datos del archivo JSON
    with open("kakuro2023top10.dat", mode="r") as file:
        datos_top = json.load(file)

    # Crear la ventana principal
    ventana_top10 = tk.Tk()
    ventana_top10.title("Top 10 Kakuro 2023")
    ventana_top10.geometry('325x350')
    ventana_top10.resizable(tk.FALSE,tk.FALSE)
    # Crear el widget Text
    texto_top = tk.Text(ventana_top10, width=40, height=20,bg=gris_claro,fg='white')
    texto_top.place(relx=0.5,rely=0.5,anchor='center')

    # Mostrar los datos en el widget Text
    texto_top.insert(tk.END, "TOP 10\n\n")
    for nivel, datos_nivel in datos_top.items():
        texto_top.insert(tk.END, f"NIVEL {nivel}:\n")
        
        # Ordenar los datos por tiempo de menor a mayor
        datos_nivel_ordenados = sorted(datos_nivel, key=lambda x: x[1])

        for i, dato in enumerate(datos_nivel_ordenados[:10], start=1):
            jugador, tiempo = dato
            texto_top.insert(tk.END, f"{i}- {jugador}: {tiempo}\n")
        texto_top.insert(tk.END, "\n")
    def on_cerrar_ventana():
        # Llamar a la función deseada al cerrar la ventana del top 10
        pausar_reanudar()

        # Cerrar la ventana principal
        ventana_top10.destroy()

    # Configurar la función a ejecutar al cerrar la ventana
    ventana_top10.protocol("WM_DELETE_WINDOW", on_cerrar_ventana)
    ventana_top10.mainloop()
    print('Top 10')
...
def guardar_juego(cronometro, squares_partida):
    #print(cronometro)
    #print(squares_partida)
    
    partida = {
        'cronometro': cronometro,
        'datos': squares_partida
    }
    
    with open('kakuro2023juegoactual.dat', 'w') as archivo:
        json.dump(partida, archivo, separators=(',', ': '), indent=2)
...
def get_partida():
    ...
    for row in range(9):
        for col in range(9):         
            valor_square = (squares[row][col]).cget("text")
            valor_square_partida = partida_actual[row][col]
            
            if (not("\\" in valor_square_partida)) and (valor_square_partida != "0"):
                    partida_actual[row][col] = valor_square   
...
def cargar_juego():
    ...
    with open('kakuro2023juegoactual.dat', 'r') as archivo:
        datos = json.load(archivo)
    
    cronometro_guardado = datos['cronometro']
    squares_partida = datos['datos']
    ...
    for i in range(9):
        ...
        for j in range(9): 
            ...
            if squares_partida[i][j] == "0":
            ...
            elif "\\" in squares_partida[i][j]: # indica una pista
                square.config(state='disabled')  
                square.configure(bg='#888888')
                         
                texto = squares_partida[i][j]
                texto_casilla = texto
                label_x = x + square.winfo_reqwidth() / 2
                label_y = y + square.winfo_reqheight() / 2
                label = tk.Label(cuadricula, text=texto, font=('Arial', 10, 'bold'), fg='black', bg='#888888')
                ...
                texto = squares_partida[i][j]
                texto_casilla = texto
                square.numero = squares_partida[i][j]
                square.configure(text=texto_casilla)
...
if True:
    ...
    boton_borrar_casilla=tk.Button(win,text='BORRAR CASILLA', fg='white',bg = gris_oscuro,font ='Dubai 10 bold',command=lambda num=0: seleccionar_numero(num))
    ...
    boton_guardar=tk.Button(win, text='GUARDAR JUEGO', fg='white',bg = gris_oscuro,font ='Dubai 10 bold',command=lambda: guardar_juego(label_cronometro.cget("text"), get_partida()))
...
def borrar_items():
    global botones_numeros, squares, iniciado, juego_iniciado
    nivel.place_forget()
...
def countdown(tiempo_restante):
    ...
    if tiempo_restante <= 0 and configuracion['reloj']==3: 
...
def pausar_reanudar():
    ...
    cronometro_pausado=not cronometro_pausado
